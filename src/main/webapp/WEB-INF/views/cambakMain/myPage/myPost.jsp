						...
						<c:choose>
							...
							<div>
								<table class="table table-hover">
									...
									<tbody>
									<c:forEach var="myPost" items="${myPostList }" varStatus="status">
										<c:if test="${myPost.board_isDelete == 'N' }">
											<tr>
												<td>${myPost.board_no }</td>
												<td>${myPost.board_category }</td>
												<c:choose>
													<c:when test="${myPost.board_category == 'CS' }">
														<td><a href="/board/cs/detail?member_id=${myPost.member_id }&no=${myPost.board_no }"> ${myPost.board_title }</a></td>
													</c:when>
													<c:when test="${myPost.board_category == 'Tip' }">
														<td><a href="/board/campingTip/view?id=${myPost.member_id }&no=${myPost.board_no }"> ${myPost.board_title }</a></td>
													</c:when>
													<c:when test="${myPost.board_category == 'humor' }">
														<td><a href="/cambakMain/board/humor/read?member_id=${myPost.member_id }&no=${myPost.board_no }"> ${myPost.board_title }</a></td>
													</c:when>
													<c:when test="${myPost.board_category == 'notice' }">
														<td><a href="/board/notice//read?member_id=${myPost.member_id }&no=${myPost.board_no }"> ${myPost.board_title }</a></td>
													</c:when>
													<c:when test="${myPost.board_category == 'QA' }">
														<td><a href="/board/qa/detail?member_id=${myPost.member_id }&no=${myPost.board_no }"> ${myPost.board_title }</a></td>
													</c:when>
													<c:when test="${myPost.board_category == 'Resell' }">
														<td><a href="/board/resell/detail?member_id=${myPost.member_id }&no=${myPost.board_no }"> ${myPost.board_title }</a></td>
													</c:when>
													<c:when test="${myPost.board_category == 'camping' }">
														<td><a href="/board/campingreview/detail?member_id=${myPost.member_id }&no=${myPost.board_no }"> ${myPost.board_title }</a></td>
													</c:when>
													<c:otherwise>
														<td>${myPost.board_title }</td>
													</c:otherwise>
												</c:choose>
												<td>${myPost.member_id }</td>
												<td><span class="sendTime" id="${status.count }">
												<fmt:formatDate value="${myPost.board_writeDate }" type="both" pattern="yyyy-MM-dd HH:mm:ss" /></span></td>
												<td>${myPost.board_viewCnt }</td>
												<td>${myPost.board_likeCnt }</td>
											</tr>
										</c:if>
									</c:forEach>
